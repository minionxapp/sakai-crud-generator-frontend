...
async function onPageChange(event) {
    // alert("halaman" + JSON.stringify(event) + event.rows)
...
//catatan ganti all data dengan getData
const searchData = async () => {
    try {
        const { data } = await custumFetch.get("/dev_projects/?page=" + pageNo.value + '&size=' + rowPerPage.value,
            {
        ...
        // rowSize.value = data.paging.zise
        // rowPerPage.value = 5
