<template>
    ...
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="results" dataKey="id" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        <h4 class="m-0">Manage dev_direktori</h4>
                        <IconField>
                ...
                <Column field="frontend" header="frontend" sortable style="min-width: 12rem"></Column>
                <Column field="backend" header="backend" sortable style="min-width: 12rem"></Column>
            ...
            <!-- //CREATE DIALOG -->
            <Dialog v-model:visible="formDialog" :style="{ width: '450px' }" header="dev_direktori Details"
                :modal="true">
                <div class="flex flex-col gap-6">
                    ...
                    <form @submit.prevent="handleSubmit">
                        ...
                        <div>
                            <label for="frontend" class="block font-bold mb-3">frontend</label>
                            <InputText rows="5" id="frontend" v-model.trim="item.frontend" required="false" fluid />
                            <small v-if="submitted && !item.frontend" class="text-red-500">frontend is required.</small>
                        </div>
                        <div>
                            <label for="backend" class="block font-bold mb-3">backend</label>
                            <InputText rows="5" id="backend" v-model.trim="item.backend" required="false" fluid />
                            <small v-if="submitted && !item.backend" class="text-red-500">backend is required.</small>
                        </div>
...
const searchData = async () => {
    ...
    try {
        const { data } = await custumFetch.get("/devdirektoris/?page=" + pageNo.value + '&size=' + rowPerPage.value + urlParam,
            {
...
async function deleteItem() {
    ...
    const myasetDelete = await custumFetch.delete('/devdirektoris/' + itemDelete.value.id,
        {
            withCredentials: true,
            headers: {
                "X-API-TOKEN": await getToken()
...
const handleSubmit = async () => {
    ...
    if (item.value.id) {
        try {
            const results = await custumFetch.put("/devdirektoris/" + item.value.id,
                {
                    username: item.value.username,
                    frontend: item.value.frontend,
                    backend: item.value.backend,
                }, {
                withCredentials: true,
                headers: {
                    "X-API-TOKEN": await getToken()
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Create dev_direktori Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
        ...
        try {
            const results = await custumFetch.post("/devdirektoris",
                {
                    username: item.value.username,
                    frontend: item.value.frontend,
                    backend: item.value.backend,
