<template>
    ...
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="results" dataKey="id" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        <h4 class="m-0">Manage Kompetensi</h4>
                        <IconField>
            ...
            <!-- //CREATE DIALOG -->
            <Dialog v-model:visible="formDialog" :style="{ width: '550px' }" header="Kompetensi Details" :modal="true">
...
const searchData = async () => {
    ...
    try {
        const { data } = await custumFetch.get("/Kompetensis/?page=" + pageNo.value + '&size=' + rowPerPage.value + urlParam,
            {
...
async function deleteItem() {
    ...
    const myasetDelete = await custumFetch.delete('/Kompetensis/' + itemDelete.value.id,
        {
            withCredentials: true,
            headers: {
                "X-API-TOKEN": await getToken()
    ...
    deleteDialog.value = false;
    itemDelete.value = {};
    toast.add({ severity: 'success', summary: 'Successful', detail: 'Kompetensi Deleted', life: 3000 });
    searchData()
}
...
const handleSubmit = async () => {
    ...
    if (item.value.id) {
        try {
            const results = await custumFetch.put("/Kompetensis/" + item.value.id,
                {
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Update Kompetensi Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
        ...
        try {
            const results = await custumFetch.post("/Kompetensis",
                {
                    kode: item.value.kode,
                    kode_job_family: item.value.kode_job_family,
                    kode_sub_job_family: item.value.kode_sub_job_family,
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Create Kompetensi Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
    }
