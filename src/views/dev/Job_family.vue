<template>
    ...
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="results" dataKey="id" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        <h4 class="m-0">Manage Job Family</h4>
                        <IconField>
            ...
            <!-- //CREATE DIALOG -->
            <Dialog v-model:visible="formDialog" :style="{ width: '550px' }" header="Job_family Details" :modal="true">
...
const searchData = async () => {
    ...
    try {
        const { data } = await custumFetch.get("/Jobfamilys/?page=" + pageNo.value + '&size=' + rowPerPage.value + urlParam,
            {
...
async function deleteItem() {
    ...
    const myasetDelete = await custumFetch.delete('/Jobfamilys/' + itemDelete.value.id,
        {
            withCredentials: true,
            headers: {
                "X-API-TOKEN": await getToken()
    ...
    deleteDialog.value = false;
    itemDelete.value = {};
    toast.add({ severity: 'success', summary: 'Successful', detail: 'Job_family Deleted', life: 3000 });
    searchData()
}
...
const handleSubmit = async () => {
    ...
    if (item.value.id) {
        try {
            const results = await custumFetch.put("/Jobfamilys/" + item.value.id,
                {
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Update Job_family Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
        ...
        try {
            const results = await custumFetch.post("/Jobfamilys",
                {
                    kode: item.value.kode,
                    nama: item.value.nama,
                    desc: item.value.desc,
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Create Job_family Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
    }
