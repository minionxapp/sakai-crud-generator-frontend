<template>
    ...
    <div>
        <div class="card">
            ...
            <DataTable ref="dt" v-model:selection="selectedItems" :value="results" dataKey="id" :paginator="true"
                ...
                <template #header>
                    <div class="flex flex-wrap gap-2 items-center justify-between">
                        <h4 class="m-0">Manage Status_training</h4>
                        <IconField>
                ...
                <Column field="aktive" header="Aktive" sortable style="min-width: 4rem"></Column>
                <Column field="desc" header="Desc" sortable style="min-width: 4rem"></Column>
                <Column field="urutan" header="Urutan" sortable style="min-width: 4rem"></Column>
            ...
            <!-- //CREATE DIALOG -->
            <Dialog v-model:visible="formDialog" :style="{ width: '550px' }" header="Status_training Details"
                :modal="true">
                <div class="flex flex-col gap-6">
                    ...
                    <form @submit.prevent="handleSubmit">
                        ...
                        <div>
                            ...
                            <!-- <InputText rows="5" id="aktive" v-model.trim="item.aktive" required="false" fluid /> -->
                            ...
                            <small v-if="submitted && !item.aktive" class="text-red-500">aktive is required.</small>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <InputNumber :showIcon="true" :showButtonBar="true" rows="5" id="urutan"
                                v-model.trim="item.urutan" required="false" fluid showButtons mode="decimal">
                            </InputNumber>
                            <small v-if="submitted && !item.urutan" class="text-red-500">urutan is required.</small>
                        </div>
...
const searchData = async () => {
    ...
    try {
        const { data } = await custumFetch.get("/Statustrainings/?page=" + pageNo.value + '&size=' + rowPerPage.value + urlParam,
            {
...
async function deleteItem() {
    ...
    const myasetDelete = await custumFetch.delete('/Statustrainings/' + itemDelete.value.id,
        {
            withCredentials: true,
            headers: {
                "X-API-TOKEN": await getToken()
    ...
    deleteDialog.value = false;
    itemDelete.value = {};
    toast.add({ severity: 'success', summary: 'Successful', detail: 'Status_training Deleted', life: 3000 });
    searchData()
}
...
const handleSubmit = async () => {
    ...
    if (item.value.id) {
        try {
            const results = await custumFetch.put("/Statustrainings/" + item.value.id,
                {
                    kode: item.value.kode,
                    nama: item.value.nama,
                    aktive: item.value.aktive,
                    desc: item.value.desc,
                    urutan: item.value.urutan,
                }, {
                withCredentials: true,
                headers: {
                    "X-API-TOKEN": await getToken()
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Update Status_training Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
        ...
        try {
            const results = await custumFetch.post("/Statustrainings",
                {
                    kode: item.value.kode,
                    nama: item.value.nama,
                    aktive: item.value.aktive,
            ...
            formDialog.value = false
            item.value = {}
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Create Status_training Success', life: 3000 });
        } catch (error) {
            console.log(error)
        }
    }
